...
from base.layers import Squeeze, Split, Transpose, Log, Broadcast2D
...
def build_fully_conv(obs_spec, act_spec, data_format="channels_first", broadcast_non_spatial=False, fc_dim=256):
  ...
  if broadcast_non_spatial:  # why would we not want to get the broadcast vector?
    non_spatial, spatial_dim = non_spatial_inputs[1], obs_spec.spaces[0].shape[1]
    non_spatial = Log()(non_spatial)
    broadcasted_non_spatial = Broadcast2D(spatial_dim)(non_spatial)  # [non_spatial, non_spatial, non_spatial*spatial, non_spatial*spatial]  (?)
    state = Concatenate(axis=1, name="state_block")([screen, minimap, broadcasted_non_spatial])  # this process seems so much easier
  else:
...
def spatial_block(name, space, cfg):
  ...
  """
  inpt = Input(space.shape, name=name + "_input")  # input to match shape of "space"
  block = Split(space.shape[0], axis=1)(inpt)  # not sure
  ...
  for i, (name, dim) in enumerate(zip(space.spatial_feats, space.spatial_dims)):  # what are these? Need to find where these are called
    if dim > 1:  # if categorical?
      block[i] = Squeeze(axis=1)(block[i])  # why specify axis?
      block[i] = Embedding(input_dim=dim, output_dim=10)(block[i])  # why 10? possible 10 categorical features so each will be a dim
      # I dont think Embedding() is quite the same as one-hot 
