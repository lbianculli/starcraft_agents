...
def main(argv):
    ...
    if args.env in utils.config.SC2_MINIGAMES_ALIASES:  #?
        args.env = utils.config.SC2_MINIGAMES_ALIASES[args.env]
    ...
    expt = utils.Experiment(args.results_dir, args.env, args.agent, args.experiment, args.restore)

    # get config files
    gin_files = utils.find_configs(args.env, os.path.dirname(os.path.abspath(__file__)))  # env_name, base_path... what's __fil__
    if args.restore:
        gin_files += [expt.config_path]
    gin_files += args.gin_files

    if not args.gpu:
        args.gin_bindings.append("build_cnn_nature.data_format = 'channels_last'")  # why does gpu play any part in this?
    ...
    sess_mgr = utils.tensorflow.SessionManager(sess, expt.path, args.ckpt_freq, training_enabled=not args.test)

    env_cls = envs.GymEnv if '-v' in args.env else envs.SC2Env
    env = env_cls(args.env, args.render, max_ep_len=args.max_ep_len)

    agent = agents.registry[args.agent](env.obs_spec(), env.act_spec(), sess_mgr=sess_mgr, n_envs=args.n_envs)
    agent.logger = utils.StreamLogger(args.n_envs, args.log_freq, args.log_eps_avg, sess_mgr, expt.log_path)
